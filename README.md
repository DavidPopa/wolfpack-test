...
Playwright collects every `tests/e2e/*.spec.ts` file. Foundation and map scenarios exercise the production web/proxy boundary; map scenarios intercept room/session responses where declared and every current browser scenario intercepts Stadia tiles. `tests/e2e/room-realtime.spec.ts` is a targeted two-context scenario that expects the dedicated fixture and proxy described by `tests/e2e/room-realtime-fixture.mjs` and `tests/e2e/room-realtime.nginx.conf`, rather than the ordinary production stack. An integrated runner must provide the runtime expected by every collected file; a fixture/runtime mismatch is a real failure and must not be hidden by filtering or skipping the spec. See [docs/TESTING.md](docs/TESTING.md) for evidence boundaries.
