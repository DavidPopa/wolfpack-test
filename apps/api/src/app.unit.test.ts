...
describe("infrastructure routes", () => {
  it("serves health independently of dependencies", async () => {
    const app = createApp({ probes: { postgres: async () => Promise.reject(), redis: async () => Promise.reject() }, readinessTimeoutMs: 50 });
    await request(app).get("/api/health").expect(200, { status: "ok", service: "api" });
  });
  it("aggregates readiness success", async () => {
    await request(createApp({ probes: passingProbes, readinessTimeoutMs: 50 })).get("/api/ready").expect(200, {
      status: "ready", dependencies: { postgres: "up", redis: "up" }
    });
  });
  it("returns a redacted failure and bounds a hanging probe", async () => {
    const app = createApp({ probes: { postgres: async () => new Promise(() => undefined), redis: async () => true }, readinessTimeoutMs: 50 });
  ...
  it("returns JSON for unknown API routes", async () => {
    await request(createApp({ probes: passingProbes, readinessTimeoutMs: 50 })).get("/api/missing")
