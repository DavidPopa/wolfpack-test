...
- Preserve the assessment PDF's visual identity rather than using default shadcn colors: white canvas, near-black ink, cool light-gray surfaces/borders, magenta-pink primary accents, and teal-green secondary/success accents. Initial PDF-render samples are approximately magenta `#E6628B` and teal `#6AC5B4`; treat these as reference colors and define semantic design tokens with darker accessible variants where text/control contrast requires them. Avoid introducing an unrelated dominant blue/purple palette; map imagery may retain its provider colors.
