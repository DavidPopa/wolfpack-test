...
Only the developer commits, including this PRD before application code. Deliver scoped changes through PRs, each with its own branch/worktree, task, QA evidence, and written review. The orchestrator identifies parallel work and dependencies. Husky gates developer commits with lint, type checks, and Jest tests; PR CI also runs Chromium E2E. See [workflow rules](../AGENTS.md#workflow).
